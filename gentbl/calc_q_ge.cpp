...
void CreateQuantumGe(CSCPS& P, double Nd, double Na)
{
...
void CalcQNdGe_Q()
{
	...
	for(i=0; i<NN; i++) 
	{
		...
		CSCPS P;
		CreateQuantumGe(P,pow(10.,logNd[i]),0.);
	...
	printarray("inc\\QNdQ_Ge.h","float Q_Nd_Q_Ge",Q,&a);
	
	printarray("inc\\CNdQ_Ge.h","float C_Nd_Q_Ge",C,&a);
}
...
void CalcQNaGe_Q()
{
	...
	for(i=0; i<NN; i++) 
	{
		...
		CSCPS P;
		CreateQuantumGe(P,0.,pow(10.,logNd[i]));
	...
	printarray("inc\\QNaQ_Ge.h","float Q_Na_Q_Ge",Q,&a);
	
	printarray("inc\\CNaQ_Ge.h","float C_Na_Q_Ge",C,&a);
}
